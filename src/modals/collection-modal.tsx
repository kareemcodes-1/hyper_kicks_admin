...
const CollectionModal = ({setOpenDialog}: {setOpenDialog: () => void;}) => {
   ...
   const formAction = async (formData: FormData) => {
        setIsLoading(true);
        if(adminInfo){
          const collectionName = editingMode ? name : formData.get("name");
          const collectionDescription = editingMode ? description : formData.get('description');
          const collectionImage = previewImage;
          const collection = {
  ...
  return (
    <DialogContent>
    <DialogHeader>
      <DialogTitle className="flex text-start mb-[2rem]">{editingMode ? 'Edit': 'Create'} Collection</DialogTitle>
  ...
  <div className="col-span-full">
    <Label htmlFor="cover-photo" className="flex text-start">
      Cover photo
    </Label>
